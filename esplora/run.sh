...
if [ "${NETWORK}" == "regtest" ] && [ -z "${NO_REGTEST_MINING}" ]; then
  if [ "${DAEMON}" != "liquid" ]; then
    /srv/explorer/bitcoin/bin/bitcoind -datadir=/data/bitcoin -daemon -listen=1 -blocknotify="pkill -USR1 electrs" -printtoconsole=1 -maxmempool=1000 -mempoolfullrbf=1 --regtest=1 --server=1 -connect=$RPC_URL -txindex=1 -rpcuser=$RPC_USER -rpcpassword=$RPC_PASSWORD -rpcbind=0.0.0.0 -rpcallowip=0.0.0.0/0 -rpcport=18443 -fallbackfee=0.00001
